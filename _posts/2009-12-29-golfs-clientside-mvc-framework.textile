...
When a request comes in, the controller looks for the array. It then examines each route, starting from the beginning. It tries to match the route's _route_ property (a regular expression) against the request's URL hash. If a match is found, the route's action method is called to handle the request. If the action returns @false@, the controller "falls through" and continues to the next route in the array. When the controller runs out of routes or an action does not return @false@ the controller resets and prepares for the next request.
...
This controller contains two routes: matching the @/home/@ and @/blog/@ URL hashes, with actions instantiating and inserting into the DOM components of type @Home@ and @Blog@, respectively.
