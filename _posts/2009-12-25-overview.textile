...
*(last) *Plugins, Mixins, And Libraries:* Golf apps can load global jQuery plugins and library scripts, as expected. But Golf also provides a flexible, yet powerful subset of ServerJS functionality that allows the use of component mixins and local namespace loading of libraries.
...
Incidentally, this model provides a nice partitioning of authorization in the backend. This means that mistakes or bugs in the frontend code can not do any harm. This is because the security of the data and backend services are all running in a completely separate machine--the server. A mistake in the frontend that causes it to fetch unauthorized data would result in only a failed XHR, not an exploit.
